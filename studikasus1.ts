...
let total = orders.map((orders) => orders.qty * orders.price);
...
orders.forEach((orders) =>
  newArray.push({ customer: orders.customer, total: orders.qty * orders.price })
);
//untuk output
newArray.forEach((orders) =>
  console.log(orders)
);
...
let daftar = orders.filter((orders) => orders.qty > 1);
console.log("Daftar pesanan yang memiliki qty lebih dari 1");
//untuk output
daftar.forEach((orders) =>console.log(orders));
